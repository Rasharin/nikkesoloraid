...
type Props = { points: Best5ChartRangePoint[]; currentSynchroLevel: number | null; fmt: (value: number) => string; compact?: boolean };
...
export default function Best5SynchroChart({ points, currentSynchroLevel, fmt, compact = false }: Props) {
  ...
  const x = (index: number) => padding + (points.length === 1 ? 0.5 : index / (points.length - 1)) * (width - padding * 2);
  ...
  const currentRangeIndex = currentSynchroLevel === null ? -1 : points.findIndex((point) => currentSynchroLevel >= point.rangeStart && currentSynchroLevel <= point.rangeEnd);
  const currentX = currentRangeIndex >= 0 ? x(currentRangeIndex) : null;
  return (
    <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-4">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="mt-4 h-auto w-full" role="img" aria-label="싱크로별 합계 딜량 그래프">
        ...
        {points.map((point, index) => <g key={point.rangeStart}><circle cx={x(index)} cy={y(point.total)} r="6" fill="currentColor" className="text-cyan-200"><title>{`싱크로 ${point.rangeStart}~${point.rangeEnd}: ${fmt(point.total)}`}</title></circle><text x={x(index)} y={height-16} textAnchor="middle" className="fill-neutral-300 text-[12px] font-medium">{point.rangeStart}~{point.rangeEnd}</text></g>)}
      </svg>
    </section>
  );
}
